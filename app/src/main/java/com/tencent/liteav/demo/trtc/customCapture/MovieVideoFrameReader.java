...
/*****************************************************************
 *
 *                 视频文件读取 MovieVideoFrameReader
 *
 *  该示例代码的作用是从一个本地的视频文件中逐帧读取视频数据，并进行解码，并产生出一帧帧的 texture。
 *  视频的解码间隔是根据视频文件中的 pts 来控制的，也就是遵循视频文件原本的播放速度。
 *
 *  视频文件的帧率是多少，每秒钟就会产生多少张纹理，但是如果您设置的 TRTCCloud 的编码帧率是 15fps（建议值），
 *  那么多余的视频帧会被 SDK 主动丢掉。
 *
 ******************************************************************/
...
public class MovieVideoFrameReader extends Thread {
    final static private String TAG = "MovieVideoFrameReader";
    ...
    public MovieVideoFrameReader(String filePath, Surface surface) {
        ...
        try {
            ...
            mMoviePlayer = new MoviePlayer(file, mSurface, null);
    ...
    public int getVideoWidth() {
        if (mMoviePlayer != null) {
            return mMoviePlayer.getVideoWidth();
        } else {
            return 0;
        }
    ...
    public int getVideoHeight() {
        if (mMoviePlayer != null) {
            return mMoviePlayer.getVideoHeight();
        } else {
            return 0;
        }
    }
    ...
    public int getVideoFps() {
        if (mMoviePlayer != null) {
            return mMoviePlayer.getVideoFps();
        } else {
            return 0;
        }
    }
    ...
    public int getVideoAngle() {
        if (mMoviePlayer != null) {
            return mMoviePlayer.getVideoAngle();
        } else {
            return 0;
        }
    }
        ...
        public MoviePlayer(File sourceFile, Surface outputSurface, FrameCallback frameCallback)
            ...
            try {
                ...
                if (format.containsKey(MediaFormat.KEY_ROTATION)) {
                    mVideoAngle = format.getInteger(MediaFormat.KEY_ROTATION);
                    if (mVideoAngle == 90 || mVideoAngle == 270) {
                        int tmpWidth = mVideoWidth;
                        mVideoWidth = mVideoHeight;
                        mVideoHeight = tmpWidth;
                    }
                }
                if (mVideoFps <= 0)
                    mVideoFps = 30;
